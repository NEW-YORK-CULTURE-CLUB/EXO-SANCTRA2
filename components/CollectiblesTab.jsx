...
const CollectiblesTab = ({
  ...
  // Filter and sort data specifically for collectibles
  let filteredData = data.filter(item => {
    const searchLower = searchQuery.toLowerCase();
    const matchesSearch = searchQuery === "" || 
      item.title?.toLowerCase().includes(searchLower) ||
      item.seriesSetName?.toLowerCase().includes(searchLower) ||
      item.manufacturerBrand?.toLowerCase().includes(searchLower) ||
      item.modelVersionSku?.toLowerCase().includes(searchLower) ||
      item.serialLotNumber?.toLowerCase().includes(searchLower) ||
      item.sku?.toLowerCase().includes(searchLower) ||
      item.location?.toLowerCase().includes(searchLower);
    
    const matchesCreator = filterCreator === "" || 
      item.manufacturerBrand?.toLowerCase().includes(filterCreator.toLowerCase());
  ...
  // Mobile card component for collectibles
    ...
    <div className="bg-card border rounded-lg p-4 space-y-3 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>{item.seriesSetName || 'N/A'}</span>
        </div>
      </div>

      {/* Collectible-specific Details Grid */}
      <div className="grid grid-cols-2 gap-3 text-sm">
        {visibleColumns.find(col => col.id === 'year')?.checked && (
          <div className="flex items-center gap-2">
            <Calendar className="h-4 w-4 text-muted-foreground" />
            <span className="text-muted-foreground">Release Year:</span>
            <span className="font-medium">{item.releaseYearEra || 'N/A'}</span>
          </div>
        )}
        ...
        {visibleColumns.find(col => col.id === 'medium')?.checked && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium line-clamp-1">{item.manufacturerBrand || 'N/A'}</span>
          </div>
        )}
      ...
      {/* Collectible-specific fields: Edition, Grade, Authentication */}
      <div className="flex flex-wrap gap-2 pt-2 border-t">
        {item.editionRunSize && (
          <Badge variant="outline" className="text-xs">
            <Hash className="h-3 w-3 mr-1" />
            Edition: {item.editionRunSize}
          </Badge>
        )}
        {item.grade && (
          <Badge variant="outline" className="text-xs">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-4">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-4">
          <span className="text-xs lg:text-sm text-muted-foreground">
            {selectedArtworks.length} of {filteredData.length} collectible(s) selected.
          </span>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Series/Set</TableHead>
                <TableHead className="cursor-pointer" onClick={() => handleSort('manufacturerBrand')}>
                  <div className="flex items-center gap-1">
                    Manufacturer <ArrowUpDown className="h-3 w-3" />
                  </div>
                </TableHead>
                {visibleColumns.find(col => col.id === 'year')?.checked && (
                  <TableHead className="cursor-pointer" onClick={() => handleSort('releaseYearEra')}>
                    <div className="flex items-center gap-1">
                      Release Year <ArrowUpDown className="h-3 w-3" />
                    </div>
                  </TableHead>
                )}
                {visibleColumns.find(col => col.id === 'size')?.checked && (
                ...
                <TableHead>Edition/Grade</TableHead>
            ...
            <TableBody>
              {paginatedData.map((item) => (
                <TableRow key={item.id} className="hover:bg-muted/50">
                  ...
                  <TableCell>{item.seriesSetName || 'N/A'}</TableCell>
                  <TableCell>{item.manufacturerBrand}</TableCell>
                  {visibleColumns.find(col => col.id === 'year')?.checked && (
                    <TableCell>{item.releaseYearEra}</TableCell>
                  )}
                  {visibleColumns.find(col => col.id === 'size')?.checked && (
                    <TableCell>
                      {item.size || (item.width && item.height ? `${item.width} x ${item.height}` : 'N/A')}
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-1 text-xs">
                      {item.editionRunSize && (
                        <span>Edition: {item.editionRunSize}</span>
                      )}
                      {item.grade && (
                        <span>Grade: {item.grade}</span>
                      )}
                      {item.serialLotNumber && (
                        <span>#{item.serialLotNumber}</span>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
