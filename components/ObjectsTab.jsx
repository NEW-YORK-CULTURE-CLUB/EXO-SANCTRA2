...
const ObjectsTab = ({
  ...
  // Filter and sort data specifically for objects
  let filteredData = data.filter(item => {
    const searchLower = searchQuery.toLowerCase();
    const matchesSearch = searchQuery === "" || 
      item.title?.toLowerCase().includes(searchLower) ||
      item.makerManufacturer?.toLowerCase().includes(searchLower) ||
      item.materialsComposition?.toLowerCase().includes(searchLower) ||
      item.modelNameCode?.toLowerCase().includes(searchLower) ||
      item.serialNumber?.toLowerCase().includes(searchLower) ||
      item.sku?.toLowerCase().includes(searchLower) ||
      item.location?.toLowerCase().includes(searchLower) ||
      item.culturalGeographicOrigin?.toLowerCase().includes(searchLower);
    
    const matchesCreator = filterCreator === "" || 
      item.makerManufacturer?.toLowerCase().includes(filterCreator.toLowerCase());
  ...
  // Mobile card component for objects
    ...
    <div className="bg-card border rounded-lg p-4 space-y-3 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>{item.makerManufacturer || 'N/A'}</span>
        </div>
      </div>

      {/* Object-specific Details Grid */}
      <div className="grid grid-cols-2 gap-3 text-sm">
        {visibleColumns.find(col => col.id === 'year')?.checked && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">{item.productionYearEra || 'N/A'}</span>
          </div>
        )}
        ...
        {visibleColumns.find(col => col.id === 'medium')?.checked && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium line-clamp-1">{item.materialsComposition || 'N/A'}</span>
          </div>
        )}
      ...
      {/* Object-specific fields: Model, Serial, Status, Origin */}
      <div className="flex flex-wrap gap-2 pt-2 border-t">
        {item.modelNameCode && (
          <Badge variant="outline" className="text-xs">
            <Package className="h-3 w-3 mr-1" />
            Model: {item.modelNameCode}
          </Badge>
        )}
        {item.serialNumber && (
          <Badge variant="outline" className="text-xs">
        ...
        {item.culturalGeographicOrigin && (
          <Badge variant="outline" className="text-xs">
            <Globe className="h-3 w-3 mr-1" />
            {item.culturalGeographicOrigin}
          </Badge>
        )}
        {item.patentsMarksHallmarks && (
          <Badge variant="outline" className="text-xs">
    ...
    <>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-2 lg:gap-4">
          <Input
            placeholder="Filter by maker/manufacturer..."
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-4">
          <span className="text-xs lg:text-sm text-muted-foreground">
            {selectedArtworks.length} of {filteredData.length} object(s) selected.
          </span>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="cursor-pointer" onClick={() => handleSort('makerManufacturer')}>
                  <div className="flex items-center gap-1">
                    Maker/Manufacturer <ArrowUpDown className="h-3 w-3" />
                  </div>
                </TableHead>
                <TableHead>Model/Code</TableHead>
                {visibleColumns.find(col => col.id === 'year')?.checked && (
                  <TableHead className="cursor-pointer" onClick={() => handleSort('productionYearEra')}>
                    <div className="flex items-center gap-1">
                      Production Year <ArrowUpDown className="h-3 w-3" />
                    </div>
                  </TableHead>
                )}
                {visibleColumns.find(col => col.id === 'medium')?.checked && (
            ...
            <TableBody>
              {paginatedData.map((item) => (
                <TableRow key={item.id} className="hover:bg-muted/50">
                  ...
                  <TableCell>{item.makerManufacturer}</TableCell>
                  <TableCell>{item.modelNameCode || 'N/A'}</TableCell>
                  {visibleColumns.find(col => col.id === 'year')?.checked && (
                    <TableCell>{item.productionYearEra}</TableCell>
                  )}
                  {visibleColumns.find(col => col.id === 'medium')?.checked && (
                    <TableCell>{item.materialsComposition}</TableCell>
                  )}
                  {visibleColumns.find(col => col.id === 'size')?.checked && (
                    <TableCell>
                      {item.size || (item.width && item.height ? `${item.width} x ${item.height}` : 'N/A')}
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-1 text-xs">
                      ...
                      {item.functionalStatus && (
                        <span>Status: {item.functionalStatus}</span>
                      )}
                      {item.culturalGeographicOrigin && (
                        <span>Origin: {item.culturalGeographicOrigin}</span>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
